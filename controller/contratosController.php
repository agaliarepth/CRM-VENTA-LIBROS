...
$f=getdate();
$res=$c->listarTodosDiferidosEsperaMes($f["mon"],$f["year"]);

if(isset($_POST["contratos"])){
	if($_POST["filtro"]=="MES") {
        $res = $c->listarTodosDiferidosEsperaMes($_POST["mes"], $_POST["anio"]);
    }
    if($_POST["filtro"]=="RANGO") {
        $res = $c->listarTodosDiferidosRango($_POST["fechainicio"],$_POST["fechafin"]);
	}
    if($_POST["filtro"]=="ACUMULADO") {
        $res = $c->listarTodosDiferidosRango("2013-01-01",$_POST["fechaacumulado"]);

    }
}
require_once("view/contratos.php");
