...
if(isset($_POST["editContrato"])&&$_POST["editContrato"]=="editContrato" ){
	...
	if($_POST["tipoventa"]=='CREDITO'){
      ...
      $de->borrarPorContrato($lastID);
	if (isset($_POST["diferido"])&& $_POST["diferido"]=='dif'){
		

        $nombre_completo=strtoupper($_POST["nombres"])." ".strtoupper($_POST["apellido_paterno"])." ".strtoupper($_POST["apellido_materno"]) ;
for($i=0; $i<$_POST["num_filas"];$i++){

   $kv1->actualizarEstadoDiferido($_POST["idkardex"][$i],$_POST["codigo"][$i],$_POST["numcontrato"],$nombre_completo, $lastID);
...
require_once("view/editContrato.php");
