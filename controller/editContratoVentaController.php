...
if(!isset($_SESSION["modulo_administracion"])){
...
if(isset($_POST["editContrato"])&&$_POST["editContrato"]=="editContrato" ){
	...
	if($_POST["tipoventa"]=='CREDITO'){
	  ...
	  $credito->numreporte=$_POST["numreporte"]; 
	  $credito->numcuenta=$_POST["numcuenta"]; 	
	  $credito->fechadoc=$_POST["fechadoc"]; 
	  $credito->tipodoc=$_POST["tipodoc"]; 
	  $credito->numdocumento=$_POST["numdocumento"];
	  $credito->montodoc=$_POST["montodoc"];  
...
header("Location:".config::ruta()."?accion=ventasAdmin");
}




require_once("view/editContratoVenta.php");
