...
if(isset($_GET["id"])&& isset($_GET["e"])&&$_GET["e"]=="adicionar"){
  ...
  foreach($det as $v){
	  ...
	  echo "addTableRow2($v[cantidad],'".$aux["titulo"]."','".$aux["tomo"]."',$aux[cif],".$t.",$v[libros_idlibros],'".$aux["codigo"]."');";
...
$("#monto_total").val(precio_total);
	   ...
	   function recalcularNota(){
		   ...
		   function Aceptar(){
				...
				if(confirm("Se registrara la Nota de Egreso... ?"))
			  ...
			  function Rechazar(){
				...
				if(confirm("Anular la nota de egreso... ?"))
					
					window.location='<?php config::ruta()?>?accion=pendientes&id=<?php echo $res["idventas"]?>&e=anular';
...
<div id="main_content">
	...
	<thead>
		...
		<td>
            <label>Envia :</label>
            <input name="nombre_envia"   size="50"  type="text"  id="nombre_envia" value="" />
        ...
        <INPUT type="text"  class="inp-form" name="tipo" value="VENTA" readonly>
          </td>
        ...
        <tr>
           ...
           <td>
        <label>Recibe :</label>	
		<input type="text" name="recibe"  size="50" readonly value="<?php echo $res["nombre"];?>"/>
            ...
            <td>
            <label>Destino:</label>
            <input type="text"  name="destino" id="destino" value="<?php echo $res["ciudad"];?>" />
            </td>
               ...
               <input type="text"  readonly="readonly" name="valor_cambio" value="<?php echo $res["cambio"];?>">
...
<table cellpadding="0"  width="90%" id="detalle" border="0">
   ...
   <input name="monto_total" size="5"  class="inp2-form" readonly  id="monto_total" /></td>
		...
		<td valign="top" colspan="8">
			<input type="button" id="bAceptar" value="Despachar" name="bAceptar" onclick="Aceptar();"/>
            			<input type="button" id="bRechazar" value="Rechazar" name="bRechazar" onclick="Rechazar();"/>

            <input type="button" id="cancelar" value="Regresar"  name="cancelar" onclick="javascript:window.location='<?php config::ruta()?>?accion=pendientes';" />
