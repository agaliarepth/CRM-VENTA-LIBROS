...
f="<tr><td><input type='text' size=5 name='numcuota[]' value='"+cuota+"'></td><td><input type='text' size=10 name='fechacuota[]' value='"+fecha+"'></td><td><input type='text' size=8 name='montocuota[]'  readonly id='montocuota' value='"+monto+"'></td></tr>";
 ...
 function validarForm(){
   ...
   confirmForm($("#wizard"),"Se guardadan los cambios en la cuenta. Desea continuar?");
...
<tr>
	...
	<td>
			<!-- start id-form -->
        <form method="post"   class="contacto"  action="<?php config::ruta()?>?accion=editCuenta" name="form" id="wizard"  >
	...
	<thead>
       ...
       <td> <label>Fecha Facturacion: </label>
           ...
           <input type="text"   name="fechaFacturado" id="fechaFacturado" class="fechas" readonly   value="<?php echo $res["fechadoc"];?>"/>
       ...
       <td> <label for="cliente">Monto Cuenta:</label>
		...
		<input type="text" class="inp2-form" name="saldo" id="saldo" readonly value="<?php echo $res["saldo"];?>" />
		</td>
          <td> <label for="cliente">Saldo_actual:</label>

		<input type="text" class="inp2-form" name="saldo_actual"  id="saldo_actual"  value="<?php echo $res["saldo"]-$sumPagos;?>" readonly />
		</td>
     ...
     <td> <label for="cliente">Cu Mensual:</label>
		...
		<input type="text" class="inp2-form" name="cuotamensual"  readonly value="<?php echo $res["montocuotas"];?>"/>
		</td>
		...
		<input type="text" class="inp-form" name="zona" value="<?php if($swr==1) echo $referencia["zona"];?>"  />
		</td>
        <td> <label for="cliente">Barrio:</label>

		<input type="text" class="inp-form" name="barrio"  value="<?php if($swr==1) echo $referencia["barrio"];?>"/>
		</td>
       <td colspan="2"> <label for="cliente">Direccion:</label>

		<input type="text" class="inp-form" name="dir"   value="<?php if($swr==1) echo $referencia["direccion"];?>" />
		</td>
        <td> <label for="cliente">Telf:</label>

		<input type="text" class="inp2-form" name="telf" value="<?php if($swr==1) echo $referencia["telf"];?>" />
		</td>

     <td colspan="2"> <label for="cliente">Lugar Cobranza:</label>

		<input type="text" class="inp-form" name="lugar" value="<?php if($swr==1) echo $referencia["lugarcobranza"];?>"/>
		</td>
        <td> <label for="cliente">Dia Cobranza:</label>

		<input type="text" class="inp2-form" name="diacobranza"  value="<?php if($swr==1) echo $referencia["diacobrar"];?>" />
		</td>
      ...
      <tr>
       ...
       <td> <label for="cliente">Verificador:</label>
		...
		<input type="text" class="inp-form" name="verificador" value="<?php if($swc==1) echo $cred["verificador"]?>" />
		</td>
      <td> <label for="cliente">Transferencia:</label>

		<input type="text" class="inp-form" name="transferencia" value="<?php if($swc==1) echo $cred["transferencia"]?>" />
		</td>
        <td colspan="2"> <label for="cliente">Supervisor:</label>

		<input type="text" class="inp-form" name="sup" value="<?php if($swc==1) echo $cred["sup"]?>" />
		</td>
         <td colspan="2"> <label for="cliente">G.C.:</label>

		<input type="text" class="inp-form" name="gc" value="<?php if($swc==1) echo $cred["gc"]?>"  />
		</td>
         <td colspan="2">
        <label for="cliente">Observaciones</label>
        <textarea name="obs" id="obs"><?php if($swc==1) echo $cred["obs"]?></textarea>
        </td>
       </tr>
...
<?php }?>
		...
		<td valign="center">
               ...
               <?php  if($swr){?>
                <input type="hidden" name="idref" value="<?php echo $referencia["idreferencias"];?>" />
               <?php }?>
    <?php  if($swc){?>
        <input type="hidden" name="idcuenta" value="<?php echo $cred["idcuentas"];?>" />
    <?php }?>
        ...
        <input type="Limpiar" value="" class="form-reset"   onclick="enviarRuta('<?php config::ruta()?>?accion=cuentas','Desea cancelar la operacion actual?.');" />
</td>
