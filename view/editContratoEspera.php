  ...
  $('input').keypress(function(e){
   ...
   $("#pu").bind('keypress', function(event)
	...
	if (event.keyCode == '13')
		...
		if(saldo<0){
			mensaje("El monto cancelado no pude ser mayor al monto total del contrato","warning");
			$(this).val("");
			$("#saldocontado").val(0);
			$(this).focus();
	    ...
	    $('#cuota_inicial').change(function () {
		   ...
		   if(parseFloat($('#cuota_inicial').val())>parseFloat($("#monto_total").val()) || $("#monto_total").val()==0){
			  ...
			  mensaje("La cuota inicial no puede ser mayor al monto Total del Contrato.","warning");
			   $('#cuota_inicial').val("");
			   $('#cuota_inicial').focus();
			   $("#saldo").val(0);
  ...
  foreach($res2 as $v){
	...
	if($res["tipoventa"]=="CREDITO"){
	   ...
	   echo"$('#idcobrador').val(".$cred["idcobrador"].");";
	    ...
	    echo"$('#saldocontado').val(".$cont["saldo"].");";
	   echo"$('#montocancelado').val(".$cont["monto"].");";
	...
	function validarContrato(){
		...
		if(!validarNumContrato()){
			 $("#wizard").css({ display: "block"});
			   $("#tabla-validacion").css({ display: "none"});
...
<td>
  <label>NUM DE CONTRATO</label><input    class="fechas"type="text" id="buscarContrato"  /></td>
   <td ><input type="button" value="VALIDAR CONTRATO" onclick="validarContrato();" style="background:#DC0E2D; font-weight:bold; font-size:11px; height:35px; color:#FFF; " /></td>
   </tr>
   </table>
  ...
  <form action="" method="post"  name="form" id="wizard" style="display:none;">
...
<table  style="margin:auto" border="0" class="detalleContrato" cellpadding="0" cellspacing="0" >
           ...
           <td><label>Nombres<span> * </span></label><input type="text" class="validate[required] text-input" data-prompt-position="topRight:-70" style="width:150px; height:20px"id="nombres" name="nombres" value="<?php echo $res["nombres"]?>"/> </td>
            <td><label>Apellido Paterno: <span> * </span></label><input type="text" class="validate[required] text-input" data-prompt-position="topRight:-70" style="width:150px; height:20px"id="apellidopaterno" name="apellidopaterno" value="<?php echo $res["apellidopaterno"]?>" /> </td>
             <td><label>Apellido Materno</label><input type="text" style="width:150px; height:20px"id="apellidomaterno" name="apellidomaterno" value="<?php echo $res["apellidomaterno"]?>" /> </td>
               <td><label>CARNET<span> * </span></label><input type="text"  class="validate[required] text-input" data-prompt-position="topRight:-70" style="width:70px; height:20px"id="carnet" name="carnet"  value="<?php echo $res["ci"]?>" /> </td>
        ...
        <tr>
            ...
            <input type="text" class="validate[custom[integer],required]" style="width:100px;  height:20px;"  id="num_pagos" name="num_pagos" value=""/>
                  ...
                  <td width="130s"><label>MONTO X PAGO <span> * </span></label><input type="text" class="validate[custom[number],required]" style="width:100px;  height:20px" id="monto_pagos" name="monto_pagos"   value=""/> </td>
              
             
			
        <td><label >MONTO COMISIONABLE<span> * </span></label><input type="text"  class="validate[required,custom[number]] text-input" data-prompt-position="topRight:-70" style="width:100px; height:20px; " id="valorcomisionable" name="valorcomisionable" value=""/> </td>
            ...
            <tr>
            <td  colspan="3" style="background-color:#FFC;"><label> NOMBRE DEL COBRADOR</label><input type="text" class="inp4-form" id="cobrador" name="cobrador" value=""/>
        ...
        <tr>
          <td >
            ...
            <input type="text" class="validate[custom[number]]" style="width:150px;  height:20px;"  id="montocancelado" name="montocancelado" value=""/>
             ...
             <td  ><label>SALDO <span> * </span></label><input type="text" class="validate[custom[number]]" style="width:100px;  height:20px" id="saldocontado" name="saldocontado"  readonly="readonly"  value="0"/>
              </td>
        ...
        <TR>
		<td >
			<input type="button" id="bEnviar" value="bEnviar" class="form-submit" name="bEnviar" onclick="validarFormulario();" />
            ...
            <input type="hidden" name="terminado" value="<?php echo $res["terminado"]?>"/>
             ...
             <input type="hidden" name="diferido" value="<?php echo $_GET["e"]?>"/>
