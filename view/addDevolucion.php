...
$("#campos tr").remove();
	  ...
	  if(res.sw==1 && nextinput>0){
            validar_fecha=1;
      ejecutarAccion("Ha cambiado el mes o el año en la fecha del documento,por lo tanto es necesario borrar los items asignados  a esta Devolucion. Desea Continuar?",$("#idcontrato").val());
        //alert(validar_fecha);
        }
        ...
        confirmForm($("#addLibros"),"Se hara efectiva la nota de devolucion en fecha <b class='resaltar'>"+$("#fecha").val()+"</b>. Desea continuar?.");
...
<div id="page-heading">
  ...
  <h1>NOTA DE DEVOLUCION > NUEVO  <table id="tabla-validacion" class="detalleContrato" style="margin: auto; border:dashed 1px; ">
<tr  style="background-color:#FFC;">
  <td >
  <label> FECHA DEVOLUCION </label><input type="text" class="fechas" id="fecha" name="fecha" value="<?php echo date("Y-m-d") ?>"/>
  <input  id="reg_btn" type="button" value="REGISTRAR FECHA"  style="background:#DC0E2D; font-weight:bold; font-size:11px; height:35px; color:#FFF; " />
  </td>
   </tr>
   </table></h1>
...
if ( count($res2) == 0) {
   ...
   <?php 
           ...
           <form method="post"   class="contacto" action=""  name="form" id="addDevolucion" style="display:none"  >
		...
		<input type="button" value="VER TODAS REMISONES" onclick="verRemisiones();" style=" background-color:#CF0; color:#000; margin-left:15px; font-size:16px; font-family:calibri; height:25px;"/>
     	...
     	<tr>
          <?php if(isset($_GET["e"])&& $_GET["e"]=="s"){?>
           <td colspan="2" ><label>VENDEDOR</label><input  class="inp4-form" type="text" readonly name="nombre_vendedor" value="<?php echo $res4["nombre_vendedor"];?>" /></td>
          <?php }else{?>
           <td colspan="2" ><label>VENDEDOR</label><input  class="inp4-form" type="text" readonly name="nombre_vendedor" value="<?php echo $vendedor["apellidos"]." ".$vendedor["nombres"];?>" /></td>
           <?php }?>
       
        
        
         <?php if(isset($_GET["e"])&& $_GET["e"]=="s"){?>
          <td colspan="2"><label>ALMACEN</label><input class="inp-form" type="text" readonly name="desc_almacen" value="<?php echo $res4["almacen"];?>" /></td>
          <?php }else{?>
           <td colspan="2"><label>ALMACEN</label><input class="inp-form" type="text" readonly name="desc_almacen" value="<?php echo $alm["descripcion"];?>" /></td>
          <?php }?>
       
            <td>
        <label>FECHA</label>
        <input  readonly type="text" class="fechas" id="verFecha"/>
...
<?php } 
 ...
 <input type="hidden" name="id_almacenes" id="id_almacenes" value="<?php echo $_POST["almacenes"];?>" />
            <input type="hidden" name="id_vendedor" id="id_vendedor" value="<?php echo $_POST["idvendedor"];?>"/>
<?php }?>
