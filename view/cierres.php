...
<div id="page-heading">
  ...
  <h1>CONTROL DE CIERRES Y APERTURAS DE MES</h1>
  <hr />
  </div>
...
<th><label for="anio">MODULO CERRADO</label><select name="modulo" id="modulo" class="inp-form">
...
<input type="hidden" name="guardar" id="guardar" value="guardar" /> 
				...
				switch($v["mes"]){
                 ...
                 <td><?php 
				if($v["modulo"]==1)echo "ALMACENES";
				if($v["modulo"]==2)echo "VENTAS";
				if($v["modulo"]==3)echo "COBRANZAS";
				
				
				?></td>
                <td><?php if($v["llave"]==0)echo"CERRADO";
				if($v["llave"]==1)echo"ABIERTO"; ?></td>
                <td><a   class="icon-unlock" href="<?php echo config::ruta();?>?accion=cierres&e=a&id=<?php echo $v["idcierres"];?>">ABRIR</a> / <a  class="icon-lock" href="<?php echo config::ruta();?>?accion=cierres&e=c&id=<?php echo $v["idcierres"];?>" >CERRAR</a></td>
                </tr>
					<?php 
